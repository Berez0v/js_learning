        ...
        arrayOfRules.forEach(rule => {
            if (rule === 'required') {
                if (!this.element.value) {
                    returnValue = 'fill in the field!';

                }
            }
            if (rule === 'mail') {
                if (!this.element.value.includes('@')) {
                    returnValue = "Please enter correct E-mail!";
                }
            ...
            if (rule === 'date') {
                if(!(new Date(this.element.value) instanceof Date && !isNaN((new Date(this.element.value)).valueOf()))){
                    returnValue = 'Please enter date in correct format (MM//DD//YYYY)'
                }
            }
            if (rule.includes('min')) {
                let temp = rule.split(":");
                if (parseInt(this.element.value) < parseInt(temp[1])) {
                    returnValue = `Value should be greater than ${parseInt(temp[1])}`;
                }
            }
    ...
    for (let i = 0; i < myArray.length; i++) {
        ...
        if (myArray[i].element.rules) {
            ...
            if (typeof(validationResult) === 'string') {
                myArray[i].element.style.border = "3px solid red";
                myArray[i].element.after(validationResult)
            } else {
                myArray[i].element.style.border = "3px solid green";
            }
        }
    }
